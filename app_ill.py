...
poly = model["poly"]              # PolynomialFeatures
ridge = model["ridge"]            # Ridge 回归模型（拟合的是 logit(y/Y_MAX)）
feature_names = model["feature_names"]
SPORE_FACTOR = model["spore_factor"]  # 目前前端不用，只是保留
Y_MAX = model["y_max"]            # 最大发病率（50%）
...
# ---------- 4. 预测函数（使用 scaler + poly + ridge） ----------
def predict_from_inputs(
    heat_hours: float,
    may_peak_spores: float,
    july_peak_spores: float,
    level_code: int,
) -> float:
    """
    预测发病率（单位：%），连续 0 ~ Y_MAX（默认 0~50）
    输入的孢子数就是“总孢子数”（已合并 3 种孢子）。
    """
    ...
    z = ridge.predict(xp)
    # 还原到发病率（0~Y_MAX）
    y_hat = Y_MAX * inv_logit(z)
    # 裁剪，防止数值边界问题
    return float(np.clip(y_hat, 0.0, Y_MAX))
